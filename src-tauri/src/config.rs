...
pub fn get_config_path(app: tauri::AppHandle) -> Result<PathBuf, tauri::Error> {
	let path = app.path().document_dir().expect("Failed to get document dir").join("Musicore/");
...
pub fn edit_music_path(new_music_path: &str, app: tauri::AppHandle) -> Result<(), String> {
	...
	fs
		::write(&config_path, &new_contents)
		.map_err(|e| format!("Failed to write config to {:?}: {}", config_path, e))?;

	let verify = fs::read_to_string(&config_path).map_err(|e| format!("Failed to verify: {}", e))?;
...
pub fn read_config(app_handle: tauri::AppHandle) -> Result<String, std::string::String> {
	let config = get_config_path(app_handle).unwrap().join("musicore.config.toml");
...
mod tests {
	...
	fn test_create_config() {
		println!(
			"{:?}",
			create_config(std::env::current_exe().unwrap().parent().unwrap().to_path_buf())
		)
	}
}
